...
% --- Revision highlighting (reviewer corrections) ---
\definecolor{fluo}{RGB}{255,255,0}   % jaune fluo
\sethlcolor{fluo}
\newcommand{\rev}[1]{\hl{#1}}        % \rev{...} = surligne les ajouts
% ----------------------------------------------------
...
% TODO(reviewer 2.11): affiliations 1 and 3 each merge two institutions in a single
% entry (an FSM-Monastir research lab + the author's home institution). Split each into
% two distinct numbered affiliations and re-map the author superscripts accordingly.
\address{%
...
% TODO(reviewer 3.3): add 4 bibitems on FPGA/DPU CNN acceleration + quantization-aware deployment surveys, then cite them here as \citep{...}.
...
\label{eq:fuzzymu}
\end{equation}
where $\alpha > 0$ represents the smoothing hyperparameter and $\pi = (\pi_1, \dots, \pi_K)$ acts as a prior distribution over the emotion classes. \rev{Both RAF-DB and FER2013 provide a single label per image and no multi-annotator metadata; Eq.~\eqref{eq:fuzzymu} is therefore stated in its general vote-count form for completeness. For a single-label corpus \mbox{$n_c = \mathbb{1}[c=y]$}, so the membership reduces to the additively-smoothed soft target \mbox{$\mu_c = (\mathbb{1}[c=y] + \alpha\pi_c)/(1+\alpha)$}, generated algorithmically from the ground-truth label, the smoothing strength \mbox{$\alpha$}, and the prior \mbox{$\pi$} --- no annotator votes are required. We set \mbox{$\alpha = 0.1$} (the standard label-smoothing value) and take \mbox{$\pi$} as the empirical class-frequency prior. Softening the one-hot target reflects the intrinsic ambiguity of facial expressions --- confusable pairs such as fear/disgust share action units --- and improves calibration on minority classes.}
...
\label{eq:fuzzy_loss}
\end{equation}
further supplemented during training by a binding divergence loss $\mathcal{L}_{\text{bind}} = D_{\text{KL}}(\mu \parallel \sigma(b))$ to align subspace representations with graded membership targets.
...
As highlighted in Fig.~\ref{fig:fleoblock1}, the fuzzy-label calculation is a training-only supervision signal that is discarded at export. \rev{The Gram--Schmidt operator, by contrast, is load-bearing at inference: naively deleting it collapses RAF-DB accuracy from \mbox{$0.858$} to \mbox{$0.073$}. Deployment is therefore a two-step process---a fold-out rewrite removes the DPU-hostile operator, and a short post-fold fine-tuning (10 epochs, no orthogonality) lets the remaining convolutions re-absorb its function, recovering accuracy to \mbox{$0.849$}. Its benefit is thus internalized into the weights by fine-tuning rather than obtained for free, while the exported graph retains only DPU-native primitives.} During inference, the network leverages the optimized feature representation directly, ensuring zero non-native computational overhead while preserving robust and interpretable facial emotion recognition performance.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%revised%%%%%
%================================revised===================================
...
Although the application of rewrite operator $\mathcal{R}$ successfully guarantees continuous single-subgraph DPU execution without CPU interruptions, removing the explicit orthogonalization constraint post-training induces a minor structural perturbation within the feature manifold. To rigorously quantify this performance penalty prior to hardware compilation, we define the fold-out accuracy degradation metric $\Delta_{\mathrm{fold}}$, measured comprehensively under both full-precision FP32 and quantized INT8 execution modes as the Eq. \eqref{eq:deltafold}.
\begin{equation}
...
\label{eq:deltafold}
\end{equation}
Because the feature representations have already internalized orthogonal properties during the multi-task training stage, this performance drop is inherently localized. To fully recover from this structural perturbation and realign the downstream convolutional filters, a lightweight fold-finetuning phase (Alg.~\ref{alg:foldout1}, line~\ref{alg:ft}) is executed over a concise duration of $T=10$ epochs on the operator-free graph. This targeted optimization step successfully closes the accuracy gap $\Delta_{\mathrm{fold}}$ from an initial degradation of $0.785\%$ down to a negligible residual of $0.009\%$, ensuring that hardware acceleration is achieved with zero compromise in recognition accuracy.
...
\begin{table}[h!]
\centering
\caption{Subspace width $d$ ablation on RAF-DB. Accuracy is insensitive to $d$ (within a $3$-point band), while the per-site FLEO parameter cost grows quadratically. We adopt $d=8$ as the accuracy--cost balance.}
\label{tab:dablation}
\setlength{\tabcolsep}{8pt}\renewcommand{\arraystretch}{1.2}
\begin{tabular}{ccc}
\toprule
