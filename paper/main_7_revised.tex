...
\usepackage{amssymb}
% ---------------------------------------------------------------------------
% Revision marking. IMPORTANT: soul's \hl is NOT used. It conflicts with the
% MDPI class (microtype letter-spacing) and crashes on ---, --, ~, \ref, \cite
% and inline math, which halts compilation after ~2 pages. \textcolor is robust
% to all of those, so the whole paper compiles. Revisions appear in blue.
% ---------------------------------------------------------------------------
\definecolor{revcolor}{RGB}{0,0,190}   % revision blue
...
\Author{Olfa ASKRI  $^{1,2,}$$^*$, Seifeddine MESSAOUD$^{3}$, Mohamed Ali HAJJAJI $^{1,4}$, and Mohamed ATRI $^{5}$}
...
% Reviewer 2, Comment 11: affiliations 1 and 3 previously merged two institutions each
% (the shared FSM-Monastir research lab + the author's home institution). They are now
% split into distinct numbered affiliations; the shared lab is affiliation 1, and the
% author superscripts above are remapped accordingly (Askri: 1,2; Hajjaji: 1,4).
% AUTHOR: verify the institution names below are exact before submission.
\address{%
$^{1}$ \quad Research Laboratory on Algebra, Theory of Numbers and Intelligent Systems, Faculty of Sciences of Monastir (FSM), Monastir University, Monastir 5019, Tunisia\\
$^{2}$ \quad National School of Electronics and Telecommunications of Sfax (ENET'Com), Sfax University, Sfax 3018, Tunisia; olfa.Askri@esen.tn\\
$^{3}$ \quad Laboratory of Condensed Matter and Nanoscience (LR11ES40), Department of Physics, Faculty of Sciences of Monastir (FSM), Monastir University, Monastir 5019, Tunisia; seifeddine.messaoud@fsm.rnu.tn\\
$^{4}$ \quad Higher Institute of Applied Sciences and Technology of Sousse, Sousse University, Sousse 4003, Tunisia; mohamedali.hajjaji@issatso.rnu.tn\\
$^{5}$ \quad Computer Engineering Department, College of Computer Science, King Khalid University, Abha 61421, Saudi Arabia; matri@kku.edu.sa}
...
\rev{Beyond facial analysis, our work connects to the broader effort of deploying signal-processing
pipelines efficiently on constrained and reconfigurable hardware---as pursued in reconfigurable
intelligent surface (RIS)-assisted mobile communications~\cite{ref_ris}, near-field metasurface
routing~\cite{ref_meta}, and orbital-angular-momentum directional modulation for secure
transmission~\cite{ref_oam}---while, on the perception side, computer-vision tasks such as 2D
human pose estimation~\cite{ref_pose} are methodologically adjacent to facial expression
recognition.   }
...
where $\Delta^{K-1}$ is the probability simplex and $\mu_c$ specifies the degree of membership of a facial image in emotion category $e_c$. When annotator consensus is absolute, $\mu$ collapses to the standard vertex one-hot vector $\mathbf{1}_{y}$.
 \rev{Here $y$ denotes the ground-truth class index and $\mathbf{1}[\cdot]$ the indicator function, equal to $1$ when its argument holds and $0$ otherwise.}
...
single-label corpus \mbox{$n_c=\mathbf{1}[c=y]$}, so Eq.~\eqref{eq:fuzzymu}
reduces to the additively-smoothed soft target
\mbox{$\mu_c=(\mathbf{1}[c=y]+\alpha\pi_c)/(1+\alpha)$}, generated
...
the one-hot vector \mbox{$\mathbf{1}[c=y]$} of the ground-truth class; (ii) add the prior term
...
\begin{table}[h!]
\centering
\caption{\rev{Hardware and software configuration for training and edge deployment.}}
\label{tab:setup}
\resizebox{\textwidth}{!}{
\begin{tabular}{lll}
\toprule
...
\multirow{5}{*}{Deployment (Hardware)}
 & Board & Xilinx ZCU104 \citep{bib2} \\
 & Accelerator & DPUCZDX8G\_ISA1\_B4096 \\
 & Operating System & PetaLinux \\
 & Vitis AI Stack & Vitis AI \citep{bib1} \\
 & Power Telemetry & INA226 / PMBus \citep{bib3} \\
\bottomrule
\end{tabular}}
\end{table}
```
...
FLEO Subspaces & $K=7$, $d=8$, P3/P4 \\
Orthogonality / Binding Weights & $0.05$ / $0.01$ \\
FLEO Channel Dropout & $0.15$ \\
Random Seeds & 3 runs \\
Fold-Finetuning & 10 epochs, FP32 \\
\bottomrule
\end{tabular}}
\end{table}
...
%======= added per Reviewer 2, Comment 9 (dedicated evaluation-metrics section) =======
% NOTE: this block is intentionally NOT wrapped in \rev{...}, because soul's \hl
% cannot span display-math (equation) environments. The whole subsection is new.
\rev{\subsubsection{Evaluation Metrics}\label{sec:metrics}
All recognition results use the detection-cast top-1 read of the emotion head. For each emotion class $c$ with true positives $\mathrm{TP}_c$, false positives $\mathrm{FP}_c$, and false negatives $\mathrm{FN}_c$, precision and recall are
\begin{equation}
P_c = \frac{\mathrm{TP}_c}{\mathrm{TP}_c + \mathrm{FP}_c}, \qquad
R_c = \frac{\mathrm{TP}_c}{\mathrm{TP}_c + \mathrm{FN}_c},
\label{eq:precrec}
\end{equation}
and the per-class $F_1$ score is their harmonic mean $F_{1,c} = 2 P_c R_c / (P_c + R_c)$. We report the unweighted macro-averages, e.g. $\text{macro-}F_1 = \tfrac{1}{K}\sum_{c=1}^{K} F_{1,c}$, which weight rare and frequent emotions equally and are therefore the primary quantities on these imbalanced benchmarks. Detection quality is summarized by the mean average precision at an IoU threshold of $0.5$ (mAP@0.5). The quantization and fold-out costs are the accuracy deltas $\Delta_{\mathrm{q}} = \mathrm{Acc}_{\mathrm{FP32}} - \mathrm{Acc}_{\mathrm{INT8}}$ (Eq.~\eqref{eq:deltaq}) and $\Delta_{\mathrm{fold}} = \mathrm{Acc}(\text{full}) - \mathrm{Acc}(\text{folded})$ (Eq.~\eqref{eq:deltafold}). All accuracy figures are reported as mean $\pm$ standard deviation over three seeds.}
...
\begin{table}[h!]
\centering
\caption{Per-class recall and $F_1$ score on RAF-DB comparing baseline YOLOv8s and YOLOv8-FLEO framework.}
\label{tab:perclass1}
\resizebox{\textwidth}{!}{
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.6\linewidth]{fig_heatmap_perclass.png}
\caption{Per-class  heatmap.}
\label{fig:perclassheat}
\end{figure}
...
\rev{In a matched-backbone comparison (both YOLOv8s), FLEO improves the hardest minority class by the largest margin---disgust recall \mbox{$+0.076$} (\mbox{$0.562 \rightarrow 0.638$}) and fear recall \mbox{$+0.095$} (\mbox{$0.527 \rightarrow 0.622$}, Table~\ref{tab:perclass1})---while overall detection stays at parity (mAP@0.5 \mbox{$0.8732$} vs.\ \mbox{$0.8727$}), confirming that the gains concentrate on the most ambiguous, low-support classes.}

\begin{figure*}[h!]
\centering
\includegraphics[width=\linewidth]{heatmap.jpg}
\caption{\rev{Grad-CAM saliency computed on the FLEO neck (P3 and P4 insertion sites) for
representative RAF-DB faces (top: input, bottom: overlay). Activation concentrates on the
eyes, brow, and mouth the facial action-unit regions that disambiguate the confusable
pairs rather than on background or contour.}}
\label{fig:gradcam}
\end{figure*}

\rev{Beyond the aggregate confusion statistics, Fig.~\ref{fig:gradcam} provides a spatial view of
\emph{where} the detector attends. We compute Grad-CAM saliency on the FLEO-augmented neck and
overlay it on representative RAF-DB faces. The activation concentrates on the eyes, brow, and
mouth---the action-unit regions that separate the confusable pairs identified above (fear vs.\
surprise, disgust vs.\ angry)---rather than on background or contour. This confirms that the
orthogonalized subspaces steer the detector toward emotion-bearing structures, giving a
mechanistic explanation for the minority-class recall gains reported in
Fig.~\ref{fig:confred}.}
...
% NOTE: table added per Reviewer 1 (d ablation); NOT wrapped in \rev{...} because
% soul's \hl cannot span a table environment (this previously would not compile).
...
\begin{table}[h!]
\centering
\caption{Deployment and compilation summary of the optimized YOLOv8-FLEO framework on the ZCU104 B4096 DPU target.}
\label{tab:deploy}
%\setlength{\tabcolsep}{6pt}\renewcommand{\arraystretch}{1.2}
\resizebox{\textwidth}{!}{
\begin{tabular}{lll}
...
% Reviewer 3, Comment 3.3 -- suggested references. AUTHORS: complete author list,
% venue, volume, pages and year in MDPI format for each entry below.
\bibitem[\protect\citeauthoryear{Ismail \emph{et al.}}{{2025}}]{ref_ris}
\rev{Ismail, A. M., Zhao, Y., Wang, Z., Guan, Y. L., \& Yuen, C. (2025). Visually Steered Reconfigurable Intelligent Surface-Assisted Mobile Communications. \textit{IEEE Antennas and Wireless Propagation Letters}, \textbf{24}(11), 4497--4501. \url{https://doi.org/10.1109/LAWP.2025.3584790}}

\bibitem[\protect\citeauthoryear{Zhao \emph{et al.}}{{2026}}]{ref_meta}
\rev{Zhao, Y., Lv, Q., Chen, Y., Ismail, A. M., Guan, Y. L., \& Yuen, C. (2026). Metasurface Router for Near-Field Multi-User Low-Interference Access Driven by Structured Wave. \textit{IEEE Transactions on Vehicular Technology} (early access), 1--6. \url{https://doi.org/10.1109/TVT.2026.3728032}}

\bibitem[\protect\citeauthoryear{Zhao \emph{et al.}}{{2026}}]{ref_oam}
\rev{Zhao, Y., Jiang, X., Liu, Z., Guan, Y. L., \& Yuen, C. (2026). Azimuth-Dependent Secure Transmission With Orbital Angular Momentum Directional Modulation. \textit{IEEE Transactions on Vehicular Technology} (early access), 1--6. \url{https://doi.org/10.1109/TVT.2026.3719765}}

\bibitem[\protect\citeauthoryear{Lin \emph{et al.}}{{2026}}]{ref_pose}
\rev{Lin, D., Zhang, Y., Yu, Y., Gao, S., Zhou, L., \& Zhao, Y. (2026). Techniques of 2D Human Pose Estimation Based on Computer Vision: A Survey. \textit{Electronics}, \textbf{15}(13), 2809. \url{https://doi.org/10.3390/electronics15132809}}
